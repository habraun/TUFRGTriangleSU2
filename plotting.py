...
def get_vertex_data(dirtype,subdir,f1,f2,N,U,V1,V2,V3,J,t1,t2,t3,m,GammaRes,MRes,KRes,shell):
    ...
    try:
        ...
        with h5py.File(filename, "r") as f:
            ...
            pvertex=  np.array(f["p"])
            cvertex=  np.array(f["c"])
            dvertex=  np.array(f["d"])
            
            pmax=  np.array((f["pmax"]))
            cmax=  np.array((f["cmax"]))
            dmax=  np.array((f["dmax"]))
    ...
    return idxlist,coords,pvertex,cvertex,dvertex,pmax,cmax,dmax,Lambda
...
def get_gap_data(dirtype,subdir,N,U,V1,V2,V3,J,t1,t2,t3,m,GammaRes,MRes,KRes,shell):
    ...
    with h5py.File(filename, "r") as f:
        ...
        gap =  np.array((f["leadingvecs1111"]))
        vals= np.array((f["leadingvals1111"]))


        coords = np.array((f["coords"]))
        
        return gap,vals,coords 
...
idxlist,coords,pvertex,cvertex,dvertex,pmax,cmax,dmax,Lambda=get_vertex_data(dirtype,subdir,f1,f2,N,U,V1,V2,V3,J,t1,t2,t3,m,GammaRes,MRes,KRes,shell)
BubblesGamma,BubblesM,Lambda=get_bubble_data(dirtype,subdir,N,U,V1,V2,V3,J,t1,t2,t3,m,GammaRes,MRes,KRes,shell)
gap,vals,coords             =get_gap_data(dirtype,subdir,N,U,V1,V2,V3,J,t1,t2,t3,m,GammaRes,MRes,KRes,shell)
...
ax1.plot(Lambda/abs(t1),pmax,label="$X=P$",color="deepskyblue",linestyle="dotted")
ax1.plot(Lambda/abs(t1),dmax,label="$X=D$",color="orange",linestyle="solid")
ax1.plot(Lambda/abs(t1),cmax,label="$X=C$",color="violet",linestyle="dashed")
ax1.set_xscale("log")
...
for k in range(0,4):
    ...
    ax4.set_title(np.round(vals[k],4))
